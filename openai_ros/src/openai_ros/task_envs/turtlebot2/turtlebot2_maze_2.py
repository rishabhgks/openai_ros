...
from openai_ros.robot_envs import turtlebot2_env_2
...
# class TurtleBot1(turtlebot2_env.TurtleBot2Env):
#     def __init__(self, ros_ws_abspath):
#         super().__init__(ros_ws_abspath)

# class TurtleBot2(turtlebot2_env.TurtleBot2Env):
#     def __init__(self, ros_ws_abspath):
#         super().__init__(ros_ws_abspath)

class TurtleBot2MazeEnv(turtlebot2_env_2.TurtleBot2Env):
    def __init__(self):
        """
        ...
        ros_ws_abspath = os.environ.get('HOME') + rospy.get_param("/turtlebot2/ros_ws_abspath", None)
        ...
        self.laser_scan_frame = {}
        self.cumulated_steps = {}
        self.new_ranges = []
        self.laser_filtered_pub = {}
        for robot in self.robot_list:
            #laser_scan = self._check_laser_scan_ready()
            # self._check_laser_scan_ready(robot)
            laser_scan = self.get_laser_scan(robot)
            while laser_scan is None:
                print("Waiting for laser scan for {}".format(robot))
            ...
            # Laser data
            self.laser_scan_frame[robot] = laser_scan.header.frame_id

            # Number of laser reading jumped
            self.new_ranges.append(int(
                math.ceil(float(len(laser_scan.ranges)) / float(self.n_observations))))

            self.cumulated_steps[robot] = 0.0

            self.laser_filtered_pub[robot] = rospy.Publisher(
                '/' + robot + '/turtlebot2/laser/scan_filtered', LaserScan, queue_size=1)

        rospy.logdebug("n_observations===>"+str(self.n_observations))
        rospy.logdebug(
            "new_ranges, jumping laser readings===>"+str(min(self.new_ranges)))
    ...
    def _init_env_variables(self):
        ...
        for robot in self.robot_list:
            self.cumulated_reward[robot] = 0.0
        ...
        for robot in self.robot_list:
            ...
            self.publish_filtered_laser_scan(robot, laser_original_data=laser_scan,
    ...
    def _set_action(self, action, robot):
        ...
        if action == 0:  # FORWARD
            ...
            angular_speed = 0.0
            self.last_action[robot] = "FORWARDS"
        elif action == 1:  # LEFT
            linear_speed = self.linear_turn_speed
            angular_speed = self.angular_speed
            self.last_action[robot] = "TURN_LEFT"
        elif action == 2:  # RIGHT
            linear_speed = self.linear_turn_speed
            angular_speed = -1*self.angular_speed
            self.last_action[robot] = "TURN_RIGHT"
        ...
        rospy.logdebug("END Set Action ==>"+str(action) +
                       ", NAME="+str(self.last_action[robot]))
    ...
    def _compute_reward(self, observations, done, robot):

        if not done:
            if self.last_action[robot] == "FORWARDS":
        ...
        self.cumulated_reward[robot] += reward
        rospy.logdebug("Cumulated_reward_" + robot + "=" + str(self.cumulated_reward[robot]))
        self.cumulated_steps[robot] += 1
        rospy.logdebug("Cumulated_steps" + robot + "=" + str(self.cumulated_steps[robot]))

        return reward

    # Internal TaskEnv Methods

    def discretize_observation(self, data, new_ranges, robot):
        """
        ...
        self.publish_filtered_laser_scan(robot, laser_original_data=data,
                                         new_filtered_laser_range=discretized_ranges)

        return discretized_ranges

    def publish_filtered_laser_scan(self, robot, laser_original_data, new_filtered_laser_range):
        ...
        self.laser_filtered_pub[robot].publish(laser_filtered_object)
